...
#define INTERRUPT_PIN 5             // D1_ Sử dụng GPIO5 để ngat. Tất cả các GPIO ngoại trừ GPIO16 đều có hỗ trợ interrupt
#define Thanh_Nhiet 4               //GPIO4 - D2
#define QUAT 12                     //
#define BOM 16                      // 
#define DHTPIN 14                   //  Kết nối chân DATA cảm biến nhiệt độ+ độ ẩm DHT11,DHT21             

#define DHTTYPE DHT21     // sử dụng cảm biến nào thì thay ở đây...DHT 21 (AM2301)
...
int MODE,FLAG_MODE,FLAG_PID,FLAG_MANUAL;
...
BLYNK_CONNECTED() {
 ...
 BLYNK_WRITE(V1){             // Chọn chế độ 1- PID, 2- Manual
   MODE = param.asInt(); // Nhận giá trị từ điện thoại

      if(MODE==2){           // Tự động vào chế độ PID
...
//  }
 ...
 BLYNK_WRITE(V10){   
   ...
   Toc_do_quat= map(value,0,100,0,1024);     // Quy đổi giá trị từ 0-100% thành 0-1024       //map(value, fromLow, fromHigh, toLow, toHigh);
 }
 BLYNK_WRITE(V12){   
...
void sendUptime()       //
{
 ...
 // Hum_ADC =((ADC_VALUE/1024)/0.767442)*100;   //map(value, fromLow, fromHigh, toLow, toHigh)
  Hum_ADC = map(ADC_VALUE,0,1024,0,100);
...
void ICACHE_RAM_ATTR TriacControl(){     //ICACHE_RAM_ATTR tra google
 // Serial.println("DA VAO CHUONG TRINH NGAT");
  if(FLAG_MODE==1){
    delayMicroseconds(Timer_1 * 10000);   // sử dụng Timer_2 để test- kiểm tra PID dùng biến Timer_1 
...
void setup()
{
    ...
    timer.setInterval(2000, sendUptime);  // Xử lý hàm sendUptime mỗi 1000ms (Co thể tăng thêm). Để tránh gửi dữ liệu liên tục lên Blynk Cloud.
}
...
void Mode_MANUAL(){
  ...
  // Mã nguồn xử lý chế độ Manual điều khiển thiết bị bằng tay
    ...
    Blynk.virtualWrite(V10, value);  // Gửi giá trị tốc độ quạt lên điện thoại
...
void loop()
{
 ...
 if(FLAG_PID==1){     //VAO CHE DO PID NHIET DO
    Mode_PID();
  }
 Nhap_Nhay(300);
